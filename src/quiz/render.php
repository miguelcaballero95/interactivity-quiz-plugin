...
wp_interactivity_state( 'create-block', [ 
	'solvedCount' => 0
] );
	...
	<ul>
		<?php foreach ( $custom_context['answers'] as $answer ) : ?>
			<li <?php echo wp_interactivity_data_wp_context( $answer ); ?> data-wp-on--click="actions.guessAttempt"
				data-wp-class--fade-incorrect="callbacks.fadedClass" data-wp-class--no-click="callbacks.noClickClass">
				<span data-wp-bind--hidden="!context.solved">
					<span data-wp-bind--hidden="!context.correct">
						<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" class="bi bi-check"
							viewBox="0 0 16 16">
							<path
